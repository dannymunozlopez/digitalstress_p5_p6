...
###Converting raw PROMIS Anxiety sum scores to t-sores
```{r}
...
###Converting raw PROMIS Depression sum scores to t-sores
```{r}
#Recode scores according to T-Score table
...
```{r}
p5_p6_fully_scored <- p5_p6_fully_scored %>%
  mutate(clinical_status = ifelse(promis_anx_tscore > 59, "clinical", "non-clinical"))
```
...
p5_p6_fully_scored <- p5_p6_fully_scored %>%
  mutate(
    anxiety_rank_p5 = ifelse(wave == "Peru 5", clinical_status, NA),
    anxiety_rank_p6 = ifelse(wave == "Peru 6", clinical_status, NA)
  )
```
...
```{r}
p5_p6_fully_scored <- p5_p6_fully_scored %>%
  mutate(clinical_status = ifelse(promis_dep_tscore > 53, "clinical", "non-clinical"))
```
...
p5_p6_fully_scored <- p5_p6_fully_scored %>%
  mutate(
    depression_rank_p5 = ifelse(wave == "Peru 5", clinical_status, NA),
    depression_rank_p6 = ifelse(wave == "Peru 6", clinical_status, NA)
  )
```
